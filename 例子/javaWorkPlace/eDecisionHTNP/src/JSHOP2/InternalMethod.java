...
public class InternalMethod extends InternalElement
{
  ...
  */
  ...
  /**A Vector of time constraint lists, each of which represent the set of time 
   * constraints for the branch of the method
   */
  private Vector<Vector<TimeConstraint>> subTcls;  
   ...
   *  @param subTclsIn
   *          a <code>Vector</code> of time constraint lists.
  */
  public InternalMethod(Predicate head, Vector<String> labelsIn, Vector<LogicalPrecondition> presIn,
                        Vector<TaskList> subsIn, Vector<Vector<TimeConstraint>> subTclsIn)
  {
    ...
    //-- Set the labels, preconditions, task list and the time constraint list defined on the tasks 
    //-- of branches in this method.
    labels = labelsIn;
    pres = presIn;
    subs = subsIn;
    subTcls=subTclsIn;
    ...
    //-- To iterate over the task list of the task decompositions.
  ...
  */
  public String toCode()
  {
    ...
    //-- Allocate the array to keep the possible time constraint lists that represent
    //-- possible decompositions of this method;
    s += "\t\tVector<Vector<TimeConstraint>> subTcls = new Vector<Vector<TimeConstraint>>();";
    s += endl + endl + endl;

    //-- For the task list of each possible decomposition,
    ...
    //-- For the time constraints list of each possible decomposition, declare it.
    for(int j =0; j<subTcls.size(); j++)
    {
    	s +="\t\tsubTcls.add(new Vector<TimeConstraint>());" + endl;
    	//-- if the time constraint list of this decomposition for the method is null;
    	if (subTcls.get(j).size() == 0)
    	{
    	    s +="\t\tsubTcls.set(" + j + ", null);" + endl;	
    	}    		
    	//-- else the time constraint list of this decomposition for the method is not null;
    	else 
    	{
    		//-- create the list of the time constraints
    		s += "\t\tsubTcls.set(" + j + ", createTimeConstraintList" + j + "());" + endl;
    	}
    }
    ...
    s += endl + "\t\tsetSubs(subsIn);" + endl;
    
    //-- Call the function that sets the method's time constraint list to the array that
    //-- was created and initialized.
    s += "\t\tsetSubTcls(subTcls);" + endl + "\t}" + endl;

    //-- For the task list of each possible decomposition, create the task list;
    for (int i = 0; i < subs.size(); i++)
    {
    ...
    //-- For the time constraint list of each possible decomposition, create the time constraint list;
    for (int j =0; j < subTcls.size(); j++)
    {
    	//-- if the time constraint list for this decomposition is not null;
    	if (subTcls.get(j).size()!=0)
    	{
    		//-- The function header
    		s += "\tVector<TimeConstraint> createTimeConstraintList" + j + "()" + endl + "\t{" + endl;
    		
    		s += "\t\tVector<TimeConstraint> retVal;" + endl + endl;
    		
    		s += "\t\tretVal =new Vector<TimeConstraint>()" + ";" + endl;
    		
    		   		
    		//-- The code that will produce this time constraint list
    		s += TimeConstraint.tclToCode(subTcls.get(j), "retVal");
    		
    		s += endl + "\t\treturn retVal;" + endl;  		
    		s += "\t}" + endl;
    	}
