...
public class InternalDomain
{
  ...
  */
  public void close(int varsMaxSize) throws IOException
  {
    ...
    s = "package JSHOP2;" + endl;
    
    s += "import java.util.Vector;" + endl + endl;
  ...
  */
   ...
   *  @throws IOException
  */
  public void commandToCode(LinkedList<Vector<Predicate>> states, LinkedList<Vector<Event>> eLists,
		  LinkedList<TaskList> taskLists, Vector<Vector<TimeConstraint>> tcls)
              throws IOException
  {
    //-- To hold the String to be written.
    String s;
    ...
    //-- For each problem, define the initial state of the problem
    for (Vector<Predicate> state : states)
    {
      s += "\tprivate static void createStatePredicates" + problemIdx1++ + "(State s)"
           + "\t{" + endl;
      ...
      for (Predicate p : state)
      {
        ...
        if (p.getHead() < constantsSize)
          s += "\t\ts.addPredicate(" + p.toCode() + ");" + endl;
      }

      s += "\t}" + endl + endl;
    }
    
    //-- For each problem, define the initial event list of the problem
    for (Vector<Event> eList : eLists)
    {
      s += "\tprivate static void createStateEventList" + problemIdx2++ + "(State s)"
           + "\t{" + endl;

      
      //-- For each event, in the initial world state of the problem
      for (Event e : eList)
      {
        //-- Check if the predicate's head appears in the domain too. If not,
        //-- we don't need to add it to the world state because it doesn't make
        //-- a difference.
    	//-- the first element of the event list is null; 
    	  if (e != null)
             s += "\t\ts.addEvent(" + e.toCode() + ");" + endl;
      }
    ...
    //-- Define the main function, getPlan().
    //-- The task net is defined in this function.
    ...
    //-- Initialize the event set of the initial state;
    s += "\t\tVector<Event> initialEventIn = new Vector<Event>();" + endl +endl;
    
    //-- Initialize the time stamp of the initial state;
    s += "\t\tdouble initialTimeIn;" + endl;
    s += "\t\tinitialTimeIn = 0.0;" + endl +endl;
    

    //-- Initialize the object that will represent the current state of the
    //-- world.
    s += "\t\tState s = new State(" + constantsSize + ", d.getAxioms() ,";
    s += "initialEventIn, initialTimeIn);" + endl;
    ...
    //-- Define the task list variable.
    s += "\t\tTaskList tl;" + endl;
    
    //-- Define the time constraint list
    s +="\t\tVector<TimeConstraint> tcl = new Vector<TimeConstraint>();" +endl;
    
    //-- Define the thread that solves the problems
    s +="\t\tSolverThread thread;" + endl + endl;

    //-- The index of the problem being solved.
    int problemIdx = 0;

    
    //-- For each problem, define the task list;
    //-- a task list for each problem;
    TaskList tl;
    //-- a time constraint list defined on this task list for each problem
    Vector<TimeConstraint> tcl;
    
    for (int i=0; i<taskLists.size(); i++)
    {
      //-- get the task list for this problem;
      tl=taskLists.get(i);
      //-- get the time constraint list for this problem;
      tcl= tcls.get(i);
      ...
      //-- Create the predicates of the world state for this problem.
      s += "\t\tcreateStatePredicates" + problemIdx + "(s);" + endl;
      
      //-- Create the event list of the world state for this problem.
      s += "\t\tcreateStateEventList" + problemIdx + "(s);" + endl;

      //-- Create the initial task list.
      s += endl + tl.getInitCode("tl") + endl;
      
      
      //-- Create the list of time constraints;
      for (TimeConstraint tc : tcl)
      {
          //-- Create the initial time constraint list associating to the task list
          s += endl + "\t\ttcl.add" + "(";
      ...
      //-- Create the initial task net.
      s += endl + "\t\ttn=new TaskNet(tl,tcl);" + endl;
      
      //-- Define the thread that will solve this planning problem.
      s += "\t\tthread = new SolverThread(tn, " + planNo + ");" + endl;
    ...
    s += "\t}" + endl + endl + "\tpublic static LinkedList<PlanElement> getFirstPlanElements() {";
    s += endl + "\t\treturn getPlans().getFirst().getPlanElements();" + endl;
    s += "\t}" + endl + "}";
  ...
  */
  ...
  public static void run(String[] args) throws Exception
  {
	  ...
	  System.out.println(endl + "Total time:\t" + total + "millisecond;");
  }
