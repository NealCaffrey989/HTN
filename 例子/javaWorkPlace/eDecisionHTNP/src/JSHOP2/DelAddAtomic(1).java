...
public class DelAddAtomic extends DelAddElement
{
  ...
  */
  ...
  /** To add this atomic delete/add element of instantaneous effects
   *  to the current state of the world.
  */
  public void addInstantEffects(State s, Term[] binding, Vector[] delAddList)
  {
   ...
   *  world.
   *  the delAddList store the predicate and the position where the predicate is deleted
  */
  public boolean delInstantEffects(State s, Term[] binding, Vector[] delAddList)
  {
    //-- Apply the binding (and execute the possible code calls) first.
    Predicate p = atom.applySubstitution(binding);
    ...
    if ((index = s.delPredicate(p)) != -1)
      ...
      //-- the delAddList store the predicate and the position where it was deleted
