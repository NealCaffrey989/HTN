...
//------------------------------- Event list ---------------------------
el  returns  [Vector<Event> retVal]
 {
    //-- The current event
    Event event = null;   
...
op :
  ...
  LP OPERATOR LP on:OPID tn = terml RP pre = lp instantDel = da instantAdd = da
     delayedDel = el delayedAdd = el
...
//------------------------------- Time point ------------------------
//-- time point
tp[Vector<TaskAtom> tasks ] returns [TimePoint retVal]
:
   TR  
    {
...
//------------------------------- Time constraint ------------------------
//-- time constraint
tc[Vector<TaskAtom> tasksIn] returns [TimeConstraint retVal]
...
//-- time constraint list
tcl[TaskList tlIn] returns [Vector<TimeConstraint> retVal]
 {
