...
public class InternalDomain
{
   ...
   *  requested from this object.规划的数量
  */
  private int planNo;

  /** A <code>Vector</code> of axioms seen so far in the domain description.
   *  Each member is of type <code>InternalAxiom</code>.公理
  */
  ...
  */
   ...
   *  problem description.常量的数量
  */
  private int constantsSize;

  /** The new line character in the platform JSHOP2 is running on.下行与行之间的分隔符相当于“\n”
  */
   ...
   *  Each member is of type <code>InternalMethod</code>.方法
  */
  private Vector<InternalMethod> methods;

  /** The <code>String</code> name of the domain.领域名字
  */
  private String name;

  /** A <code>Vector</code> of operators seen so far in the domain description.
   *  Each member is of type <code>InternalOperator</code>.操作
  */
  private Vector<InternalOperator> operators;

  /** The parser object that will parse this domain.领域知识剖析器
  */
  public JSHOP2Parser parser;

  /** A <code>Vector</code> of <code>String</code> names of primitive tasks
   *  seen so far in the domain.原子任务
  */
  private Vector<String> primitiveTasks;

  /** The <code>String</code> name of the planning problem.问题名称
  */
   ...
   *  @throws IOException输入输出异常,数据库输出时，可以修改File fin这个参数
  */
  ...
  */
  ...
  {
    //-- To hold the String to be written.Java代码
    String s;

    //-- JSHOP2 classes should be imported first.添加引用
    s = "import JSHOP2.*;" + endl + endl;

    //-- Produce the classes that represent the operators.添加操作类
    for (int i = 0; i < operators.size(); i++)
      s += operators.get(i).toCode();

    //-- Produce the classes that represent the methods.添加方法类
    for (int i = 0; i < methods.size(); i++)
      s += methods.get(i).toCode();

    //-- Produce the classes that represent the axioms.添加公理类
    for (int i = 0; i < axioms.size(); i++)
      s += axioms.get(i).toCode();

    //-- Produce the class that represents the domain itself.添加领域知识类
    s += "class " + name + " extends Domain" + endl + "{" + endl;

    //-- Take care of the user-defined external code calls first by
    //-- instantiating an  object of that class to do the calculations.调用外部程序，初始化外部类
    //for (int i = 0; i < calcs.size(); i++);;jshop2原版代码，现在屏蔽
    //{
    ...
    //-- Produce the constructor for the class that represents this domain.构造函数
    ...
    //-- Produce the array that maps constant symbols to integers.常量
    s += vectorToCode(constants, "constants");
    //-- Produce the array that maps compound tasks to integers.复合任务
    s += vectorToCode(compoundTasks, "compoundTasks");
    //-- Produce the array that maps primitive tasks to integers.原子任务
    ...
    //-- corresponding compound task.方法
    ...
    //-- corresponding primitive task.操作
    ...
    //-- which start with the corresponding constant symbol.公理
    ...
    //应用自定义编译
    try {
		AdvancedCompiler.complie(s, name);
	} catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}

	InfoStore.putdomaininfo(compoundTasks, constants, primitiveTasks);
	
	if(1==1){//原版代码，现在屏蔽	
	 //-- Open the file with the appropriate name.写入硬盘，保存为Java代码
    BufferedWriter dest = new BufferedWriter(new FileWriter("../src/"+name + ".java"));
   ...
   *  with the domain the planning problem is defined in.读取生成的TXT文件
   *
   *  @throws IOException
  */
  ...
  //自定义函数，取代commandInitialize() 
  public void cmdInitialize()
  {
    if(InfoStore.DomainInstance==null)
    	return;
    else{
    	constants=InfoStore.DomainInstance.getConstants();
    	constantsSize=constants.size();
    	compoundTasks=InfoStore.DomainInstance.getCompoundTasks();
    	primitiveTasks=InfoStore.DomainInstance.getPrimitiveTasks();
    }
  }
  

  /** This function writes the Java code necessary to produce these planning
   *  problems at run time in the appropriate file.编辑problem的Java代码
   *
   *  @param states 状态
   *          the list of initial state of the world, one per each planning
   *          problem.
   *  @param taskLists 任务列表
  ...
  */
              ...
              throws IOException
  {
    ...
    //-- but not in the domain description.定义常量
    ...
    //-- Set the values of elements of that array.\转义符
    ...
    //-- Define the main function.编写获取规划函数
    ...
    //-- represents the domain.Domain里已经定义了的常量
    ...
    //-- JSHOP2 algorithm.初始化JSHOP2中的领域domain与状态state
    s += endl + "\t\tJSHOP2.initialize(d, s);" + endl + endl;

    //-- Define the task list variable and the thread that solves the problems.定义任务列表和线程解决者
    s += "\t\tTaskList tl;" + endl + "\t\tSolverThread thread;" + endl + endl;

    //-- The index of the problem being solved.已经解决了的问题索引
    problemIdx = 0;
    ...
    for (TaskList tl : taskLists)
    {
      ...
      //-- Create the world state for this problem.创建问题的状态
      s += "\t\tcreateState" + problemIdx + "(s);" + endl;

      //-- Create the initial task list.创建任务列表
      s += endl + tl.getInitCode("tl") + endl;

      //-- Define the thread that will solve this planning problem.定义解决这个计划问题的线程
      s += "\t\tthread = new SolverThread(tl, " + planNo + ");" + endl;

      //-- Start the thread that will solve this planning problem.开始线程
      s += "\t\tthread.start();" + endl + endl;

      //-- Wait till thread is done, since JSHOP2's data members are static and
      //-- can handle only one problem at a time.线程调度
    ...
    //自定义编译
    try {
		AdvancedCompiler.complie(s, probName);
	} catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
    ...
    //-- Open the file with the appropriate name.
      dest = new BufferedWriter(new FileWriter("../src/"+probName + ".java"));
  ...
  */
  ...
  /** This function returns the number of axioms in this domain.获取公理的数目
   *
   *  @return
  ...
  */
  ...
  /** This function returns the number of methods in this domain.获取方法的数量
   *
   *  @return
  ...
  */
  ...
  /** This function returns the <code>String</code> name of this domain.获取领域名称
   *
   *  @return
  ...
  */
  public static void main(String[] args) throws Exception
  {
    ...
    //-- Handle the number of solution plans the user wants to be returned.根据输入参数确定计划数量
    ...
    //-- Check the number of arguments.异常输入时发生
    ...
    //-- If this is a planning problem, call the 'command' rule in the parser.写problems
    if (args.length == 2)
      (new InternalDomain(new File(args[1]), planNo)).parser.command();
    //-- If this is a planning domain, call the 'domain' rule in the parser.写domain
    else
      (new InternalDomain(new File(args[0]), -1)).parser.domain();
  }
  //--自己加的代码，用于外部实现编译
  public static void Comples(String[] args)
  {
	  //-- The number of solution plans to be returned.
	  int planNo = -1;
   ...
   *  a given file.从输入流中读取字符列表，返回列表长度
   *
   *  @param src
   *          the input file.
   *  @param list
  ...
  */
  ...
  /** To set the name of this planning domain.设置问题名称
   *
   *  @param nameIn
  ...
  */
   ...
   *  <code>String</code>s.编写数组型的Java代码
   *
   *  @param list
