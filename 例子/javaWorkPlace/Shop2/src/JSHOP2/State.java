...
public class State
{
  ...
  */
   ...
   *          the variable symbols in the given predicate.切入点，获取变量及其描述
  */
  public Term[] nextBinding(Predicate p, MyIterator me)
  {
    ...
    String s=p.toString();
    String[] res=p.toString().substring(1, p.toString().length()-1).split(" ");
    String[] res1=s.substring(res[0].length(), s.length()-1).split(" ");
	InfoStore.VarInstance.putvarinfo(res[0],res1);
   ...
   // for(int i=0;i<me.vec.size();i++)
      //	System.out.println("me.vec["+i+"]:"+me.vec.get(i));
